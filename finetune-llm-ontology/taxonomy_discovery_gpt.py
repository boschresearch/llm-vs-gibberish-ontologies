...
def make_example(thoughts, answer, cot):
    return f"""```json
{{
    "thoughts": "{thoughts}",
    "answer": {answer}
...
"""
    if args.shots == "one":
        answer_example = f"""Example query:
...
Expected answer:
{make_example("a potato is an edible tuber, which fits the definition of root vegetable.", "true", args.cot)}
"""
    elif args.shots == "few":
        answer_example = f"""Here are a few examples that can aid you.
...
Expected answer:
{make_example("A music instrument is not a living organism, hence it is not an animal.", "false", args.cot)}
...
Expected answer:
{make_example("A car is a vehicle, as it is a road vehicle.", "true", args.cot)}
"""
    else:
        answer_example = ""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, help="Model name.", default="gpt35")
    ...
    parser.add_argument("--cot", action="store_true", help="Whether to provide thoughts or not.")
